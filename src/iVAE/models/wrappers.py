...
def IVAE_wrapper(
    ...
    print(
        "training on {}".format(torch.cuda.get_device_name(device) if cuda else "cpu")
    )
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, factor=0.1, patience=scheduler_tol, verbose=True
    )
        ...
        if S is not None:
            print(
                "epoch {}/{} \tloss: {}\tperf: {}".format(
                    epoch, max_epochs, logger.get_last("elbo"), logger.get_last("perf")
                )
            )
        else:
            print(
                "epoch {}/{} \tloss: {}".format(
                    epoch, max_epochs, logger.get_last("elbo")
                )
            )
...
def TCL_wrapper(
    ...
    with tf.Graph().as_default():
        data_holder = tf.placeholder(
            tf.float32, shape=[None, sensor.shape[0]], name="data"
        )

        # Build a Graph that computes the logits predictions from the
        # inference model.
        logits, feats = tcl_core.inference(
            data_holder, list_hidden_nodes, num_class=num_segment
        )
        ...
        with tf.Session() as sess:
            ...
            tensor_val = tcl_eval.get_tensor(
                sensor, [preds, feats], sess, data_holder, batch=256
            )
