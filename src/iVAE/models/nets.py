...
def _check_inputs(size, mu, v):
    ...
    if size is None and mu is None and v is None:
        ...
        raise ValueError(
            "Given invalid inputs: size={}, mu_logsigma={})".format(size, (mu, v))
        )
...
class cleanIVAE(nn.Module):
    def __init__(
        ...
        self.decoder_var = 0.1 * torch.ones(1)
        # encoder params
        self.g = MLP(
            data_dim + aux_dim,
            latent_dim,
    ...
    def elbo(self, x, u, N, a=1.0, b=1.0, c=1.0, d=1.0):
        ...
        logqs_tmp = log_normal(
            z.view(M, 1, d_latent), g.view(1, M, d_latent), v.view(1, M, d_latent)
        )
        logqs = torch.logsumexp(logqs_tmp.sum(dim=-1), dim=1, keepdim=False) - np.log(
            M * N
        )
        logqs_i = (
            torch.logsumexp(logqs_tmp, dim=1, keepdim=False) - np.log(M * N)
        ).sum(dim=-1)
...
class ModularIVAE(nn.Module):
    ...
    def elbo(self, x, u):
        ...
        if self.anneal_params:
            a, b, c, d, N = self._training_hyperparams
            M = z.size(0)
            log_qz_tmp = self.encoder.log_pdf(
                z.view(M, 1, self.latent_dim),
                encoder_params,
                reduce=False,
