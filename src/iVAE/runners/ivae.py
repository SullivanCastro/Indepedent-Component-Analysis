        ...
        for epoch in range(1, config.epochs + 1):
            ...
            for i, data in enumerate(data_loader):
                ...
                if factor:
                    ...
                    D_z_pperm = D(z_pperm)
                    D_tc_loss = 0.5 * (
                        F.cross_entropy(D_z, zeros) + F.cross_entropy(D_z_pperm, ones)
                    )
            ...
            print(
                "==> Epoch {}/{}:\t train loss: {:.6f}\t train perf: {:.6f} \t full perf: {:,.6f}".format(
                    epoch, config.epochs, train_loss, train_perf, perf_all
                )
            )
